...
AgentHandle.prototype.prepareAgentEventQueue = function (callback) {
    ...
    var queueName = common.dotjoin(
        this.resource + '-client', this.uuid, 'events', common.genId());

    self.log.info('Waiting for task events on queue: ' + queueName);
...
AgentHandle.prototype.sendTask = function (task, msg, callback) {
    ...
    self.log.info('Publishing message to routing key: "' + routingKey + '"');
    self.log.info(JSON.stringify(msg, null, '  '));
