...
ThrottledQueue.prototype._shouldShift = function () {
  this.log.debug('Checking if we should shift ('+this.msgCount+'/'+this.maxMsgCount+")");
  if (this.shifting) {
    ...
    this.log.debug('Not shifting because we\'re at the maximum number of concurrent provisions (' + this.msgCount + ").");
    return false;
  }
  else {
